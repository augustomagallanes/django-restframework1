...
def comic_list_filtered_api_view(request):
    min_price = request.query_params.get('min_price', 5.00)
    _queryset = Comic.objects.filter(price__gt=min_price)
    _data = list(_queryset.values()) if _queryset.exists() else []

    return Response(data=_data, status=status.HTTP_200_OK)

@api_view(http_method_names=['GET'])
def comic_list_filtered_stock_api_view(request):
    min_price = request.query_params.get('stock_qty', 3)
    _queryset = Comic.objects.filter(price__gte=min_price)
    _data = list(_queryset.values()) if _queryset.exists() else []
